 ...
 * EXAMPLES OF USE:
 *
 * $('.news_item').quickpaginate(); // would paginate all items on the page with the class of 'news_item'
 *
 * $('div#quick_slideshow img').quickpaginate(); // would paginate all img elements within the div with an id of 'quick_slideshow'
 *
 * // Paginate everything with the class of "news_item",
 * // 5 per 'page' and without the page counter.
 * $('.news_item').quickpaginate( { perpage: 5, showcounter: false } );
 *
 ...
 * $('img, #info').quickpaginate({ pager : $("#pager_here") });
 *
 */
...
jQuery.fn.quickpaginate = function( settings ) {
    ...
    var setNav = function()
    {
        ...
        var pagerNav = $('<ul><li><span class="qp_btn '+settings.prev+'">&laquo; Prev</span><span class="qp_btn '+settings.next+'" >Next &raquo;</span></li></ul>');

        $(settings.pager).append( pagerNav );

        if ( settings.showcounter )
        {
            var counter = '<span class="'+settings.counter+'"><span class="'+settings.pagenumber+'"></span> of <span class="'+settings.totalnumber+'"></span></span>';
