...
class form
{
	...
	public function __construct($arr, $elements = array(), $action = '', $formName = '', $method = 'post', $conf = array(), $use_ids = true, $target = null, $id = null)
	{
		...
		zf::addJS('zf.form', BASE_URL.'zf/js/form.js');
	}
				...
				switch ($val['type']) {
					...
					case 'audio':
                        if ($_FILES && $_FILES[$key]['error'] != 4) {
							$val['value'] = $_FILES ? $_FILES[$val['name']]['tmp_name'] : BASE_URL.'img/no_image.gif';
	...
	{
        if ($inp['crop']) {
            zf::addJS('image_cropper_select', BASE_URL.'zf/js/jquery.imgareaselect.js');
            zf::addJS('image_cropper', BASE_URL.'zf/js/form.image_cropper.js');
            zf::addCSS('image_cropper', BASE_URL.'zf/imagearea/imgareaselect-animated.css');
	...
	public function get_images_input($inp, $attrStr)
	{
		if (isset($inp['link'])) {
			...
			$ret = '';
			zf::addJS('zf_images', BASE_URL.'zf/js/files.js');
			if (!empty($inp['value'])) {
	...
	{
		zf::addJS('ckfinder', BASE_URL.'zf/ckfinder/ckfinder.js');
		return '<input type="text" name="'.$inp['name'].'" value="'.$inp['value'].'" style="padding-right: 18px;">'
		.'<img src="'.BASE_URL.'cms/img/icons/delete.png'.'" style="margin-left: -26px; margin-right: 10px; vertical-align: middle;" '
    ...
    {
        $ret = '';
        zf::addJS('zf_form', BASE_URL.'zf/js/jquery.form.js');
        zf::addJS('zf_MultiFile', BASE_URL.'zf/js/jquery.MultiFile.js');
        zf::addJS('zf_blockUI', BASE_URL.'zf/js/jquery.blockUI.js');
        if (!empty($inp['value'])) {
            zf::addJS('lytebox_js', BASE_URL.'zf/js/lytebox_322cmod1.3.js');
            zf::addCSS('lytebox_css', BASE_URL.'zf/css/lytebox_322cmod1.3.css');
	...
	public function get_video_input($inp, $attrStr)
	{
		...
		if ($inp['value'] && strrpos($inp['value'], '.tmp') != strlen($inp['value']) - 4) {
			zf::addJS('swfobject', BASE_URL.'zf/js/swfobject.js');
	...
	public function get_audio_input($inp, $attrStr)
	{
		$repl = array();
		if ($inp['value'] && strrpos($inp['value'], '.tmp') != strlen($inp['value']) - 4) {
			zf::addJS('swfobject_ap', BASE_URL.'zf/audioplayer/swfobject_ap.js');
			zf::addJS('audio-player', BASE_URL.'zf/audioplayer/audio-player.js');
	...
	{
		zf::addJS('form.tselect', BASE_URL.'zf/js/form.tselect.js');
	...
	{
		zf::addJS('form.multitext', BASE_URL.'zf/js/form.multitext.js');
        if (!empty($inp['value'])) {
			$first=true;
	...
	{
		zf::addJS('form.doublemultitext', BASE_URL.'zf/js/form.doublemultitext.js');
		$ret='
		<table width="90%">
			<tr>
	...
	public function get_metroselect_input($inp, $attrStr)
	{
		...
		$ret .= '<div class="metromap" style="display: none; position: relative;">
			...
			<img src="'.(!empty($inp['map_img'])? $inp['map_img'] : BASE_URL.'zf/img/metro_moscow.gif').'">
			<a href="#" class="station" style="display: none;">
				<img wigth="11" height="12" class="placeholder" src="'.BASE_URL.'zf/img/null.gif'.'" alt="Станция метро" title="Станция метро">
				<img wigth="11" height="12" class="marker" style="display: none;" src="'.BASE_URL.'zf/img/pointer.gif'.'" alt="Станция метро" title="Станция метро">
			</a>
		</div>';

		$ret .= '</div>';
		zf::addJS('form.metroselect', BASE_URL.'zf/js/form.metroselect.js');
		$stations = array();
	...
	public function get_shtml_input($inp, $attrStr)
	{
		...
		$ret = "<textarea name=\"{$inp['name']}\" {$attrStr}>{$inp['value']}</textarea>
			<script type=\"text/javascript\">
				...
				$is.height = 200;
				//$is.css=BASE_URL.'zsms/css/main.css';
				$is.mode = 'XHTMLBody';
				$is.REPLACE('".$this->getId($inp)."');
			</script>
		";
        zf::addJS('wysiwyg', BASE_URL.'zf/wysiwyg/innovaeditor.js', false, true);
		return $ret;
	}
	...
	{
		zf::addJS('refreshcaptcha', BASE_URL.'zf/js/form.captcharefresh.js');
		$ret  = "<div style='position:relative;'><img style='cursor: pointer;' src=\"/captcha/".$inp['conf']['background_color']."/?".rand()."\" $attrStr /><img style='display:none; position:absolute; top:0; left:0;' src='" . BASE_URL.'zf/img/loading.gif' . "' alt=''/></div><br />";
		unset($inp['value']);
	...
	{
		include_once BASE_URL . 'zf/ckeditor/ckeditor.php';
		require_once BASE_URL . 'zf/ckfinder/ckfinder.php';
		zf::addJS('ckedito', BASE_URL.'zf/ckeditor/ckeditor.js');
		zf::addJS('ckfinder', BASE_URL.'zf/ckfinder/ckfinder.js');
		$id = $this->getId($inp);
		$ck = 'ck'.$inp['name'];
        $ck = str_replace('[', '', $ck);
		...
		CKFinder.setupCKEditor( $ck, '".(!empty($inp['ckfinder']) ? $inp['ckfinder'] : BASE_URL.'zf/ckfinder/')."' );
		</script>";
    ...
    public function get_datetime_input($inp, $attrStr)
    {
        ...
        zf::addCSS('dynDateTime', BASE_URL.'zf/css/calendar-blue.css');
        return $ret;
    }
	...
	public function get_time_input($inp, $attrStr)
	{
		...
		";
        zf::addJS('ui', BASE_URL.'zf/js/jquery-ui-1.7.1.custom.min.js');
		zf::addJS('timepicker', BASE_URL.'zf/js/jquery.ui.timepicker.js');
        zf::addCSS('ui', BASE_URL.'zf/css/jquery-ui-1.8.14.custom.css');
		zf::addCSS('timepicker', BASE_URL.'zf/css/jquery.ui.timepicker.css');
		return $ret;
	}
	...
	public function get_geo_input($inp, $attrStr)
	{
		...
		zf::addJS('form.geo', BASE_URL.'zf/js/form.geo.js');
		$id = $this->getId($inp);
		$strname = '';
